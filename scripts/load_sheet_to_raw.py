#!/usr/bin/env python3
"""Извлекает данные из Google Sheets и загружает в raw.data в Supabase/Postgres."""
import sys
import asyncio
import os
import hashlib
...
async def main():
    if len(sys.argv) < 2:
        print("Использование: python scripts/load_sheet_to_raw.py <spreadsheet_id> [range]")
        sys.exit(2)
    ...
    print(f"📥 Извлечение {spreadsheet_id} {range_name} ...")
    records = await fetch_google_sheets(spreadsheet_id, range_name)
    print(f"✅ Получено {len(records)} строк. Загрузка в raw.data ...")
    rows = []
    ...
    print(f"💾 Загружено {len(rows)} строк в raw.data.")
