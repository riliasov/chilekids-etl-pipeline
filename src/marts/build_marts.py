...
async def build_campaigns_summary():
    ...
    if not any('campaign_id' in it for it in items if isinstance(it, dict)):
        return
    try:
        # lightweight pandas-free aggregation
        groups = {}
        for it in items:
...
async def build_financials():
    ...
    # Aggregate Total RUB per month and Type (Доход/Расход).
    # Only include rows where Type is 'Доход' or 'Расход' and ignore obviously-bad dates (before 2005).
    sql = (
        "WITH s AS ("
    ...
    for r in rows:
        try:
            # Round totals to whole RUB for the mart (nearest integer)
            try:
                total_val = float(r['total_rub'])
            except Exception:
                total_val = 0.0
            total_rounded = int(round(total_val))
            await execute(
...
async def build_expenses_by_category():
    ...
    # Aggregate Total RUB by Category for 'Расход' (Expense)
    # We look for 'Category' or 'Категория' in the payload.
    sql = (
        "WITH s AS ("
        " SELECT "
        "   COALESCE(payload->>'Category', payload->>'Категория', 'Uncategorized') AS category,"
        "   (payload->>'Total RUB')::numeric AS total_rub,"
        "   trim(payload->>'Type') AS type"
        " FROM staging.data"
        " WHERE payload ? 'Total RUB'"
        " )"
        " SELECT category, SUM(total_rub) AS total_rub"
        " FROM s"
    ...
    for r in rows:
        try:
            total_val = float(r['total_rub'] or 0)
            total_rounded = int(round(total_val))
            await execute(
