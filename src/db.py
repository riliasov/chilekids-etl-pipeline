    ...
    if _pool is None:
        ...
        try:
            _pool = await asyncio.wait_for(
                asyncpg.create_pool(dsn=str(settings.POSTGRES_URI), min_size=min_size, max_size=max_size), timeout=30.0
            )
...
def load_service_account_info() -> dict | None:
    ...
    try:
        ...
        # The instruction refers to a change in src/sheets.py, but the provided snippet
        # attempts to modify this function in src/db.py with unrelated code.
        # To maintain syntactic correctness and avoid introducing undefined variables,
        # the original 'return None' is kept here.
        # The 'strict=True' for zip is not applicable in this file's context.
        return None
